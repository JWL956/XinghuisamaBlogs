        ...
        return (
                <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
                        <form onSubmit={login} className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl">
                                  <h1 className="text-2xl font-black text-white mb-6 text-center">956 在线管理台</h1>h1>
                                  <input
                                                type="password"
                                                value={password}
                          ...
                          {msg && <p className={`mt-4 text-sm ${msg.type === 'err' ? 'text-red-400' : 'text-green-400'}`}>{msg.text}</p>p>}
                        </form>form>
                </div>div>
              );
  }
          ...
          <div className="min-h-screen bg-slate-950 text-slate-200 px-4 py-8">
                <div className="max-w-4xl mx-auto space-y-6">
                        <div className="flex items-center justify-between">
                                  <h1 className="text-2xl font-black text-white">956 在线管理台</h1>h1>
                                  <button
                  ...
                  {/* 上传照片 */}
                        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                                  <h2 className="text-lg font-bold text-white mb-4">📷 上传照片</h2>h2>
                                  <label className="block w-full cursor-pointer border-2 border-dashed border-slate-700 hover:border-indigo-500 rounded-xl p-6 text-center transition-colors">
                                              <span className="text-sm text-slate-400">点击选择图片（jpg / png / gif / webp，单张 ≤ 8MB）</span>span>
                                              <input
                                                              type="file"
                                                              accept="image/*"
                                                              className="hidden"
                          ...
                          {uploading && <p className="text-sm text-indigo-400 mt-3">上传中，请稍候（将提交到 GitHub）...</p>p>}
                          {uploadedUrl && (
                  ...
                  {/* 照片墙数据 */}
                        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                                  <div className="flex items-center justify-between mb-4">
                                              <h2 className="text-lg font-bold text-white">🖼️ 照片墙数据（albums）</h2>h2>
                                              <button
                                                              onClick={() => save('albums')}
                                  ...
                                  <textarea
                                              ...
                                              />
                                  <p className="text-xs text-slate-500 mt-2">结构：[{'{'} id, title, description, cover, date, photos: [{'{'} url, caption {'}'}] {'}'}]。保存后约 1~2 分钟自动上线。</p>p>
                        </section>section>
                
                  {/* 项目数据 */}
                        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                                  <div className="flex items-center justify-between mb-4">
                                              <h2 className="text-lg font-bold text-white">🚀 项目数据（projects）</h2>h2>
                                              <button
                                  ...
                                  <textarea
                                              ...
                                              />
                                  <p className="text-xs text-slate-500 mt-2">结构：[{'{'} id, name, description, icon, githubUrl, tags: [] {'}'}]。保存后约 1~2 分钟自动上线。</p>p>
                        </section>section>
                </div>div>
          </div>div>
        );
