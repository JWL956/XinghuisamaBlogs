...
// 网易云音乐外链播放器默认歌曲ID（周杰伦 - 晴天）
const DEFAULT_NETEASE_SONG_ID = "1824020514";
  ...
  if (playlist.length === 0 || !currentSong) {
    return (
      <div
        ...
        className="h-full w-full rounded-3xl bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-xl p-4 flex flex-col items-center justify-center transition-all duration-700 hover:scale-[1.02] cursor-pointer overflow-hidden"
      >
        <div className="w-full flex flex-col items-center">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-bold text-slate-700 dark:text-slate-300">网易云音乐</span>
          </div>
          <iframe
            frameBorder="no"
            border="0"
            ...
            width="100%"
            height="86"
            src={`//music.163.com/outchain/player?type=2&id=${DEFAULT_NETEASE_SONG_ID}&auto=0&height=66`}
            className="rounded-lg shadow-md"
            onClick={(e) => e.stopPropagation()}
          />
          <p className="text-[10px] text-slate-400 mt-2">点击播放器控制播放</p>
        </div>
      </div>
    );
  }
